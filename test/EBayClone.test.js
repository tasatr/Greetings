...
contract("EBayClone: add product", () => {
  describe("sellProduct(string)", () => {
    it("sets product", async () => {
      const ebay = await EBayClone.deployed()

      const afterSell = await ebay.sellProduct.call("First book", "A lovely book to read first", 5);
      console.log(afterSell);

      const actual = await ebay.getNumberOfProducts();
      console.log(actual);

      assert.equal(actual, 1, "product count was not updated");
    });
  });
});
